...
class PackageChecker:
    ...
    def _get_osv_details(self, vuln_id):
        """Fetch detailed information for an OSV vulnerability."""
    ...
    def check_security(self):
        ...
        try:
            ...
            self.logger.debug(f"OSV API Response: {response.text}")
            if response.status_code == 200:
                data = response.json()
                if "vulns" in data:
                    for vuln in data["vulns"]:
                        details = self._get_osv_details(vuln["id"])
                        vulnerabilities.append(
                            {
                                ...
                                "advisory": (
                                    details.get("details")
                                    if details
                                    else vuln.get("summary", "No description available")
                                ),
                                "published": (
                                    details.get("published") if details else None
                                ),
                                "modified": (
                                    details.get("modified") if details else None
                                ),
                                "aliases": (
                                    details.get("aliases", []) if details else []
                                ),
                                "references": (
                                    details.get("references", []) if details else []
                                ),
                            }
                        )
        except Exception as e:
    ...
    def _check_cve(self):
        ...
        params = {"keywordSearch": self.package_name, "keywordExactMatch": True}

        response = requests.get(self.nvd_api, params=params)
        self.logger.debug(f"NVD API Response: {response.text}")
        if response.status_code == 200:
            data = response.json()
            vulnerabilities = []
                    ...
                    vulnerabilities.append(
                        {
                            ...
                            "advisory": cve.get("descriptions", [{}])[0].get(
